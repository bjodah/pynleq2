...
for CONDA_PY in 27 34 35; do
    ssh $PKG@$SERVER <<EOF
source /etc/profile
cp -r ~/public_html/conda-recipes/conda-recipe-$VERSION/ /tmp/conda_build/
cd /tmp/conda_build/
echo "--- setup.py2016-03-18 11:46:42.204949821 +0100">setup-url.patch
echo "+++ setup.py.22016-03-18 12:01:40.596925262 +0100">>setup-url.patch
echo "@@ -84 +84 @@">>setup-url.patch
echo "-    NLEQ2_URL = os.environ.get('PYNLEQ2_NLEQ2_ROOT_URL', None)">>setup-url.patch
echo "+    NLEQ2_URL = MYSTRING">>setup-url.patch
sed -i -E -e "s/source:/source:\n  patches:\n    - setup-url.patch"
CONDA_PY=$CONDA_PY CONDA_NPY=110 conda-build .
EOF
done
